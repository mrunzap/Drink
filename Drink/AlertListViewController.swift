...
class AlertListViewController: UITableViewController {
    ...
    @IBAction func addAlertButtonAction(_ sender: UIBarButtonItem) {
        ...
        // 화면 내에서 데이터 이동할 때는 weak self 즉, 순환참조를 할 필요가 없다.
        // self weak하는 이유가 자식뷰가 닫혔을 때, 해당 데이터 누수를 막기 위해서이다.?
        addAlertVC.pickDate = { [weak self] date in
            ...
            UserDefaults.standard.set(try? PropertyListEncoder().encode(self.alerts),
                                      forKey: "alerts")
